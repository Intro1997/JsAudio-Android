...
function wptGainNodeTest2() {
  ...
  const { computeSNR } = require("js_tests/wpt_test/helper/audit-util.js");
  ...
  // duration.
  let noteSilence = Math.floor(0.02 * sampleRate) / sampleRate;
    ...
    function (task, should) {
      ...
      context
        ...
        .then((buffer) => {
          ...
          // followed by silence.
          let bufferDurationFrames = Math.ceil(
            bufferDurationSeconds * context.sampleRate
          );
          ...
          should(actual0, "Left output from gain node").beCloseToArray(
            reference0,
            { relativeThreshold: 1.1877e-7 }
          );
          should(actual1, "Right output from gain node").beCloseToArray(
            reference1,
            { relativeThreshold: 1.1877e-7 }
          );
          ...
          should(snr0, "Left SNR (in dB)").beGreaterThanOrEqualTo(148.71);
          should(snr1, "Right SNR (in dB)").beGreaterThanOrEqualTo(148.71);
        })
        .then(() => task.done());
    }
  );
...
module.exports = { wptGainNodeTest2 };
