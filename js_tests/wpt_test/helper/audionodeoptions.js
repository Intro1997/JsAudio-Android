 ...
 * 2. use RangeError instead of NotSupportedError
 *    - nodejs do not have NotSupportedError
 */
...
function testAudioNodeOptions(should, context, nodeName, expectedNodeOptions) {
  if (expectedNodeOptions === undefined) expectedNodeOptions = {};
  let node;
  ...
  }, "new " + nodeName + "(c, {channelCount: " + testChannelCount + "})").notThrow();
  should(node.channelCount, "node.channelCount").beEqualTo(testChannelCount);

  if (
    expectedNodeOptions.channelCount &&
    ...
    }, "new " + nodeName + "(c, {channelCount: " + testChannelCount + "})").throw(
      DOMException,
    ...
    }, "(new " + nodeName + "(c, {channelCount: " + testChannelCount + "})).channelCount = " + testChannelCount).notThrow();
  } else {
    ...
    [0, 99].forEach((testValue) => {
      ...
      }, `new ${nodeName}(c, {channelCount: ${testValue}})`).throw(
        DOMException,
        // "NotSupportedError"
        "RangeError"
      );
  ...
  }, "new " + nodeName + '(c, {channelCountMode: "' + testChannelCountMode + '"}').notThrow();
  ...
  if (
    ...
    ["max", "clamped-max", "explicit"].forEach((testValue) => {
      if (testValue !== expectedNodeOptions.channelCountMode.value) {
        ...
        }, `new ${nodeName}(c, {channelCountMode: "${testValue}"})`).throw(
          DOMException,
        ...
        }, `(new ${nodeName}(c, {channelCountMode: "${testValue}"})).channelCountMode = "${testValue}"`).notThrow();
      }
    });
  } else {
    ...
    testValues.forEach((testValue) => {
      ...
      should(
        ...
        "node.channelCountMode after valid setter"
      ).beEqualTo(testValue);
    });
    ...
    }, "new " + nodeName + '(c, {channelCountMode: "foobar"}').throw(TypeError);
    should(
      node.channelCountMode,
      "node.channelCountMode after invalid setter"
  ...
  if (
    ...
    ["speakers", "discrete"].forEach((testValue) => {
      if (testValue !== expectedNodeOptions.channelInterpretation.value) {
        should(() => {
          node = new window[nodeName](
            context,
            Object.assign({}, expectedNodeOptions.additionOptions, {
              channelInterpretation: testValue,
            })
          );
        }, `new ${nodeName}(c, {channelInterpretation: "${testValue}"})`).throw(
          DOMException,
          expectedNodeOptions.channelCountMode.exceptionType
        );
      } else {
        ...
        }, `(new ${nodeName}(c, {channelInterpretation: "${testValue}"})).channelInterpretation = "${testValue}"`).notThrow();
      }
    });
  } else {
    ...
    should(node.channelInterpretation, "node.channelInterpretation").beEqualTo(
      "speakers"
    );
    ...
    }, "new " + nodeName + '(c, {channelInterpretation: "foobar"})').throw(
      TypeError
    );
    should(
...
function testDefaultConstructor(should, name, context, options) {
  ...
  should(node.numberOfInputs, options.prefix + ".numberOfInputs").beEqualTo(
    options.numberOfInputs
  );
  should(node.numberOfOutputs, options.prefix + ".numberOfOutputs").beEqualTo(
    options.numberOfOutputs
  );
  should(node.channelCount, options.prefix + ".channelCount").beEqualTo(
    options.channelCount
  );
  should(node.channelCountMode, options.prefix + ".channelCountMode").beEqualTo(
...
function testDefaultAttributes(should, node, prefix, items) {
  items.forEach((item) => {
    ...
    if (attr instanceof AudioParam) {
      should(attr.value, prefix + "." + item.name + ".value").beEqualTo(
        item.value
      );
    } else {
