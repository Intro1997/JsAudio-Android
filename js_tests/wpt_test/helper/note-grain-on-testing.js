/**
 * Adaptive Changes：
 * 1. define context, renderedData in test as global object
 *    - cannot export these objects from note-grain-on-testing.js as a variable
 */
...
function createSignalBuffer(context, f) {
  ...
  let signalLength = Math.floor(
    1 + sampleRate * (numberOfTests * grainOffsetStep + duration)
  );
...
function verifyStartAndEndFrames(startEndFrames, should) {
  ...
  for (let k = 0; k < startFrames.length; ++k) {
    ...
    if (startFrames[k] != expectedStart) ++errorCountStart;
    if (endFrames[k] != expectedEnd) ++errorCountEnd;
