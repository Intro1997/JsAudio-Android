/**
 * Adaptive Changes：
 * 1. make delayTimeSeconds global
 *    - used in delaynode-max-default-delay.js
 */
...
function checkDelayedResult(renderedBuffer, toneBuffer, should) {
  ...
  for (let i = 0; i < n; ++i) {
    if (i < delayTimeFrames) {
      ...
      if (renderedData[i] != sourceData[j]) {
        should(renderedData[i], "Actual data at frame " + i).beEqualTo(
          sourceData[j]
        );
        success = false;
        break;
  ...
  should(
    success,
    "Delaying test signal by " + delayTimeSeconds + " sec was done"
