function createTestBuffer(context, sampleFrameLength) {
  ...
  for (let i = 0; i < sampleFrameLength; ++i) channelData[i] = i;
...
function checkSingleTest(renderedBuffer, i, should) {
  ...
  if (test.description) {
    ...
    if (test.offsetFrame) description += " with offset " + test.offsetFrame;
  ...
  if (test.renderFrames) framesToTest = test.renderFrames;
  else if (test.durationFrames) framesToTest = test.durationFrames;
  ...
  should(
    renderedData.slice(
      offsetFrame + framesToTest,
      offsetFrame + testSpacingFrames
    ),
