...
function testStartStop(should, node, options) {
  ...
  nonFiniteValues.forEach((time) => {
    should(() => {
      node.start(time);
    }, `start(${time})`).throw(TypeError);
  });

  should(() => {
    node.stop();
    // }, "Calling stop() before start()").throw(DOMException, "InvalidStateError");
  }, "Calling stop() before start()").throw(DOMException, "Error");
  ...
  }, "Calling start() twice").throw(DOMException, "Error");
  should(() => {
    node.stop(-1);
  ...
  nonFiniteValues.forEach((time) => {
    should(() => {
      node.stop(time);
    }, `stop(${time})`).throw(TypeError);
  });
}

module.exports = { testStartStop };
