/**
 * Adaptive Changes：
 * 1. make sampleRate, buffer, bufferFrameLength, testSpacingFrames, testSpacingSeconds, totalRenderLengthFrames global
 *    - these object are used in checkAllTests without being passed as parameters
 */

const testName = "wptAudioBufferSourceNodeTest7<audiobuffersource-start.js>";
const {
  waitUntilTestComplete,
  requireReload,
} = require("js_tests/wpt_test/helper/polyfile.js");

async function wptAudioBufferSourceNodeTest7() {
  ...
  const {
    createTestBuffer,
    checkAllTests,
  } = require("js_tests/wpt_test/helper/audiobuffersource-testing.js");
  ...
  audit.define("Tests AudioBufferSourceNode start()", function (task, should) {
    ...
    let context = new OfflineAudioContext(
      1,
      totalRenderLengthFrames,
      sampleRate
    );
    buffer = createTestBuffer(context, bufferFrameLength);

    for (let i = 0; i < tests.length; ++i) runLoopTest(context, i, tests[i]);
...
module.exports = { wptAudioBufferSourceNodeTest7 };
