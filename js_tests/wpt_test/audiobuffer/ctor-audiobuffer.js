...
function wptAudioBufferTest1() {
  ...
  audit.define("required options", (task, should) => {
    ...
    should(() => {
      buffer = new AudioBuffer({ length: 21, sampleRate: context.sampleRate });
    }, "buffer0 = new AudioBuffer({length: 21, sampleRate: " + context.sampleRate + "}").notThrow();
    ...
    should(buffer.sampleRate, "buffer0.sampleRate").beEqualTo(
      context.sampleRate
    );
    ...
    }, "buffer1 = new AudioBuffer(" + "{numberOfChannels: 3, length: 1, sampleRate: 48000})").notThrow();
    // Verify the buffer has the correct values.
  ...
  audit.define("invalid option values", (task, should) => {
    ...
    }, "new AudioBuffer(" + JSON.stringify(options) + ")").throw(
      DOMException,
      // "NotSupportedError"
      "RangeError"
    );
  ...
  audit.define("default constructor", (task, should) => {
    ...
    }, "buffer = new AudioBuffer(" + JSON.stringify(options) + ")").notThrow();
  ...
  audit.define("valid constructor", (task, should) => {
    ...
    should(buffer.sampleRate, "buffer.sampleRate").beEqualTo(
      options.sampleRate
    );
    ...
    }, "buffer.getChannelData(" + options.numberOfChannels + ")").throw(
      DOMException,
      // "IndexSizeError"
      "RangeError"
    );
...
module.exports = { wptAudioBufferTest1 };
