 ...
 * 2. comment SharedArrayBuffer test
 *    - NodeJs does not support SharedArrayBuffer yet
...
function wptAudioBufferTest3() {
  ...
  function shouldBeRamp(should, testName, x, startValue, length, startIndex) {
    let k;
  ...
  audit.define("copyFrom-exceptions", (task, should) => {
    ...
    }, "0: buffer = context.createBuffer(" + numberOfChannels + ", " + bufferLength + ", context.sampleRate)").notThrow();
    should(() => {
    ...
    }, "3: buffer.copyFromChannel(x, -1)").throw(
      DOMException,
      // "IndexSizeError"
      "RangeError"
    );
    ...
    }, "4: buffer.copyFromChannel(x, " + numberOfChannels + ")").throw(
      DOMException,
      // "IndexSizeError"
      "RangeError"
    );
    ...
    }, "6: buffer.copyFromChannel(x, 0, " + bufferLength + ")").notThrow();

    should(() => {
      buffer.copyFromChannel(x, 3);
    }, "7: buffer.copyFromChannel(x, 3)").throw(
      DOMException,
      // "IndexSizeError"
      "RangeError"
    );
    ...
    // const shared_buffer = new Float32Array(
    //   new WebAssembly.Memory({ shared: true, initial: 1, maximum: 1 }).buffer
    // );
    // should(() => {
    //   buffer.copyFromChannel(shared_buffer, 0);
    // }, "8: buffer.copyFromChannel(SharedArrayBuffer view, 0)").throw(TypeError);

    // should(() => {
    //   buffer.copyFromChannel(shared_buffer, 0, 0);
    // }, "9: buffer.copyFromChannel(SharedArrayBuffer view, 0, 0)").throw(
    //   TypeError
    // );
  ...
  audit.define("copyTo-exceptions", (task, should) => {
    ...
    }, "2: buffer.copyToChannel(x, -1)").throw(
      DOMException,
      // "IndexSizeError"
      "RangeError"
    );
    ...
    }, "3: buffer.copyToChannel(x, " + numberOfChannels + ")").throw(
      DOMException,
      // "IndexSizeError"
      "RangeError"
    );
    ...
    }, "5: buffer.copyToChannel(x, 0, " + bufferLength + ")").notThrow();

    should(() => {
      buffer.copyToChannel(x, 3);
    }, "6: buffer.copyToChannel(x, 3)").throw(
      DOMException,
      // "IndexSizeError"
      "RangeError"
    );
    ...
    // }, "7: buffer.copyToChannel(SharedArrayBuffer view, 0)").throw(TypeError);

    // should(() => {
    //   buffer.copyToChannel(shared_buffer, 0, 0);
    // }, "8: buffer.copyToChannel(SharedArrayBuffer view, 0, 0)").throw(
    //   TypeError
    // );
  ...
  audit.define("copyTo-validate", (task, should) => {
    ...
    }, "buffer = createConstantBuffer(context, " + bufferLength + ", [" + initialValues + "])").notThrow();
...
module.exports = { wptAudioBufferTest3 };
